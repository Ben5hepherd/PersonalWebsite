...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('should send a POST request and return a User', () => {
      const mockUserDto: UserDto = { username: 'testuser', password: 'password' };
      const mockUser: User = { id: '1', username: 'testuser', passwordHash: 'hash' };

      service.register(mockUserDto).subscribe(user => {
        expect(user).toEqual(mockUser);
      });

      const req = httpMock.expectOne(`${configServiceMock.apiUrl}/api/auth/register`);
  ...
  describe('login', () => {
    it('should send a POST request and return a token', () => {
      ...
      const req = httpMock.expectOne(`${configServiceMock.apiUrl}/api/auth/login`);
      expect(req.request.method).toBe('POST');
      expect(req.request.body).toEqual(mockUserDto);
      req.flush(mockToken);
    });
